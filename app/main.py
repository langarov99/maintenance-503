    ...
    try:
        ...
        if _name_supplier in _name_db_map:
            ...
            if name_db.is_loaded:
                ...
                # Bomar: fill missing EANs from bomar-barcodes.xlsx
                if _name_supplier == "bomar":
                    _bomar_bc = get_bomar_barcode_map(str(DATA_DIR))
                    if _bomar_bc:
                        ean_filled = 0
                        for rec in records:
                            if not rec.ean and rec.product_code:
                                ean = _bomar_bc.get(rec.product_code.upper())
                                if ean:
                                    rec.ean = ean
                                    ean_filled += 1
                        if ean_filled:
                            logger.info("bomar: barcode map filled %d missing EANs", ean_filled)
